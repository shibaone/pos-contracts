...
contract Drainable is DepositManagerStorage {
    ...
    function drainErc20(address[] calldata tokens, uint256[] calldata values, address destination)
        external
        onlyGovernance
    {
        require((tokens.length == values.length), "invalid input");
        for (uint256 i = 0; i < tokens.length; i++) {
            require(IERC20(tokens[i]).transfer(destination, values[i]), "Transfer failed");
        }
    }

    function drainErc721(address[] calldata tokens, uint256[] calldata values, address destination)
        external
        onlyGovernance
    {
        require((tokens.length == values.length), "invalid input");
