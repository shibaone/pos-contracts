...
contract ValidatorShareUpgraded is IValidatorShare, ERC20NonTradable, OwnableLockable, Initializable {
    ...
    // NEW EVENT: Admin event for consumed unbond records (legacy)
    // 0 for legacy
    ...
    /**
     * NEW FUNCTION: Admin function to consume legacy unbond records for a specific user
    ...
    function adminConsumeLegacyUnbond(address user) external onlyOwner returns (uint256) {
        ...
        require(unbond.shares > 0, "No unbond record found");

        // Check if the unbond is eligible for withdrawal
        require(
            unbond.withdrawEpoch.add(stakeManager.withdrawalDelay()) <= stakeManager.epoch(),
            "Unbond period not completed"
        );

        uint256 amount = withdrawExchangeRate().mul(unbond.shares).div(_getRatePrecision());

        // Update withdrawal pool state
        withdrawShares = withdrawShares.sub(unbond.shares);
        withdrawPool = withdrawPool.sub(amount);

        // Clear the unbond record
        delete unbonds[user];

        // Transfer funds to the user
        require(stakeManager.transferFunds(validatorId, amount, user), "Transfer failed");

        // Emit event for transparency
        emit AdminConsumedUnbond(
            validatorId,
            user,
            0, // legacy unbond nonce is 0
            amount,
            unbond.shares,
            block.timestamp,
            msg.sender
